...
class ArticleController extends Controller
{
	...
	public function admin_post_complete(ArticlePostRequest $request) {
		$form = $request->only(['name', 'content']);
		$form += array('reply_id'=>0);
	...
	public function admin_edit_complete(ArticlePostRequest $request, Article $article) {
		$form = $request->only(['name', 'content']);
	...
	public function admin_delete_complete(Request $request, Article $article) {
		//ユーザーのroleを確認する
		if(Auth::user()) {						// ログインしているユーザー
			if(Auth::user()->role !== 'admin') {
				return redirect('/admin/login');
